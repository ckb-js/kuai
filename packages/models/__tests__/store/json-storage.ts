...
import { JSONStorage } from '../../src/store'
...
describe('test json storage', () => {
  ...
  describe('only witness exist', () => {
    ...
    it('more than one layer', () => {
      const storage = new JSONStorage<{ witness: { lay: {a: number; b: string; c: boolean; d: boolean; e: bigint }}}>()
      const witness = { a: 1, b: 'b', c: true, d: false, e: BigInt('100')}
  ...
  describe('data and witness exist', () => {
    ...
    it('more than one layer', () => {
      const storage = new JSONStorage<{ data: number, witness: { lay: {a: number; b: string; c: boolean; d: boolean; e: bigint }}}>()
      const witness = { a: 1, b: 'b', c: true, d: false, e: BigInt('100')}
